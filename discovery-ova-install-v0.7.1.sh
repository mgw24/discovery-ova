...
EOF
  ...
  local sso_ready=0
  for _ in {1..60}; do
    if curl -fsS --max-time 3 http://127.0.0.1:8088/auth/realms/discovery-ova/.well-known/openid-configuration >/dev/null 2>&1 \n       && curl -fsS --max-time 3 http://127.0.0.1:4180/ping >/dev/null 2>&1; then
      sso_ready=1
      break
    fi
    sleep 3
  ...
  if (( sso_ready == 0 )); then
    ...
    docker logs --tail 80 discovery-ova-oauth2-proxy 2>&1 | tee -a "$LOG_FILE" || true
    warn "Keycloak logs follow:"
    docker logs --tail 80 discovery-ova-keycloak 2>&1 | tee -a "$LOG_FILE" || true
    die "OAuth2 Proxy did not become ready after Keycloak/Nginx startup."
  fi
