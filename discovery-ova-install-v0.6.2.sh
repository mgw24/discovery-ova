...
discovery_outbound_ok() {
  ...
  if command -v curl >/dev/null 2>&1; then
    curl -4 --interface "$dev" -fsS --connect-timeout 2 --max-time 4 -o /dev/null https://connectivity-check.ubuntu.com/ >/dev/null 2>&1 && return 0
    curl -4 --interface "$dev" -fsS --connect-timeout 2 --max-time 4 -o /dev/null http://archive.ubuntu.com/ >/dev/null 2>&1 && return 0
  fi
  ping -I "$dev" -c1 -W2 1.1.1.1 >/dev/null 2>&1
}
...
ensure_dirs_user() {
  ...
  install -d -m 0750 -o root -g discovery-ova "$DISCOVERY_OVA_ROOT" "$COMPOSE_DIR" "$CONFIG_DIR" "$PORTAL_DIR" "$APP_DIR"
...
EOF
  ...
  if [[ "$ans" == y ]]; then
    ...
    while true; do
      ...
      safe_name="${name//-/_}"
      prompt_value "FlashArray management hostname/IP[:port]" "" address valid_pure_address "Use a DNS name or IPv4 address with an optional valid TCP port."
...
<a class="card" href="https://$HOSTNAME_SHORT.local:8443/" target="_blank" rel="noopener noreferrer"><b>LibreNMS</b><span>Network discovery, SNMP polling and alerting.</span><i class="tag">network monitor</i></a>
<a class="card" href="https://$HOSTNAME_SHORT.local:8444/smokeping/" target="_blank" rel="noopener noreferrer"><b>SmokePing</b><span>Remote latency and packet-loss history.</span><i class="tag">latency</i></a>
...
<a class="card" href="https://$HOSTNAME_SHORT.local:8447/" target="_blank" rel="noopener noreferrer"><b>Grafana</b><span>Metrics dashboards and centralized syslog views.</span><i class="tag">observability</i></a>
...
<a class="card" href="https://$HOSTNAME_SHORT.local:8445/" target="_blank" rel="noopener noreferrer"><b>LibreSpeed</b><span>Browser-based throughput testing.</span><i class="tag">speed</i></a>
...
<footer>Discovery-OVA $VERSION · $HOSTNAME_SHORT.local · Default timezone: $TIMEZONE</footer>
<script>
