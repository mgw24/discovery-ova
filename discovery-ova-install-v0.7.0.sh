...
OAUTH2_PROXY_IMAGE="quay.io/oauth2-proxy/oauth2-proxy:v7.15.4"
...
collect_settings() {
  ...
  if [[ -z "${OPERATOR_PASSWORD:-}" ]]; then
    prompt_secret "Password for Discovery-OVA SSO operator '$OPERATOR_USER'" OPERATOR_PASSWORD 12
  elif (( existing )); then
  ...
  KEYCLOAK_CLIENT_SECRET="${KEYCLOAK_CLIENT_SECRET:-$(random_secret)}"
...
ensure_dirs_user() {
  ...
  install -d -m 0750 -o root -g root "$DATA_DIR/db" "$DATA_DIR/redis" "$DATA_DIR/guacamole-db" "$DATA_DIR/keycloak-db"
  install -d -m 0750 -o root -g discovery-ova "$CONFIG_DIR/guacamole" "$CONFIG_DIR/blackbox" "$CONFIG_DIR/telegraf" "$CONFIG_DIR/redfish" "$CONFIG_DIR/keycloak" "$CONFIG_DIR/oauth2-proxy"
...
configure_login_banner() {
  stage "5B" "Login banner and management URLs"
  write_file /etc/update-motd.d/99-discovery-ova 0755 root:root <<'EOF'
#!/usr/bin/env bash
# Discovery-OVA dynamic login banner. Uses the current management IP only;
# no DNS or mDNS resolution is required.
set -u
CONF=/etc/discovery-ova/discovery-ova.conf
[[ -r "$CONF" ]] && source "$CONF"
mgmt_if="${MGMT_IF:-}"
operator="${OPERATOR_USER:-chili}"
ip=""
if [[ -n "$mgmt_if" ]]; then
  ip="$(ip -4 -o addr show dev "$mgmt_if" scope global 2>/dev/null | awk 'NR==1{split($4,a,"/");print a[1]}')"
fi
if [[ -z "$ip" ]]; then
...
======================================================================
 ...
 Monitoring
   LibreNMS    : https://$ip:8443/
   SmokePing   : https://$ip:8444/smokeping/
   ntopng      : https://$ip:8446/
   Grafana     : https://$ip:8447/

 Diagnostics
   LibreSpeed  : https://$ip:8445/
   Scanner     : https://$ip/scanner/
   Diagnostics : https://$ip/diagnostics/
   Capture     : https://$ip/capture/

 Remote / Infrastructure
   Guacamole   : https://$ip/guacamole/
   Monitoring  : https://$ip/remote/

 Appliance
   Network     : https://$ip/network/
   Status      : https://$ip/status/

 DNS/mDNS is optional; use the management IP above for appliance access.
======================================================================
BANNER
EOF

  # Also provide a short pre-login console/SSH identification banner without
  # embedding a potentially stale DHCP address.
  install -d -m 0755 /etc/issue.d
  cat > /etc/issue.d/discovery-ova.issue <<EOF
Discovery-OVA $VERSION - infrastructure discovery and diagnostics appliance
After login, the current management IP and web URLs are displayed automatically.
EOF
  chmod 0644 /etc/issue.d/discovery-ova.issue
}
...
write_sso_configs() {
  ...
  [[ -n "$ip" ]] || die "Cannot configure SSO without a management IPv4 address on $MGMT_IF."

  jq -n \
    --arg user "$OPERATOR_USER" \
    --arg pass "$OPERATOR_PASSWORD" \
    --arg secret "$KEYCLOAK_CLIENT_SECRET" \
    --arg redirect "https://$ip/oauth2/callback" \
    --arg origin "https://$ip" \
    '{realm:"discovery-ova",enabled:true,displayName:"Discovery-OVA",registrationAllowed:false,resetPasswordAllowed:true,rememberMe:true,loginWithEmailAllowed:true,clients:[{clientId:"discovery-ova-proxy",name:"Discovery-OVA SSO Proxy",enabled:true,protocol:"openid-connect",publicClient:false,secret:$secret,standardFlowEnabled:true,directAccessGrantsEnabled:false,redirectUris:[$redirect],webOrigins:[$origin],attributes:{"post.logout.redirect.uris":($origin+"/*")}}],users:[{username:$user,enabled:true,emailVerified:true,firstName:"Discovery",lastName:"Operator",email:($user+"@discovery-ova.local"),credentials:[{type:"password",value:$pass,temporary:false}]}]}' \
    > "$CONFIG_DIR/keycloak/realm.json"
  chmod 0644 "$CONFIG_DIR/keycloak/realm.json"

  cat > "$CONFIG_DIR/oauth2-proxy/oauth2-proxy.cfg" <<EOF
provider = "oidc"
oidc_issuer_url = "https://$ip/auth/realms/discovery-ova"
client_id = "discovery-ova-proxy"
client_secret = "$KEYCLOAK_CLIENT_SECRET"
redirect_url = "https://$ip/oauth2/callback"
email_domains = [ "*" ]
...
cookie_name = "_discovery_ova_sso"
...
EOF
  chmod 0644 "$CONFIG_DIR/oauth2-proxy/oauth2-proxy.cfg"
}
...
services:
  ...
  ntopng:
    ...
    command: ["--community", "-i", "\${CAPTURE_IF}", "-w", "127.0.0.1:3000", "--disable-login=1", "-d", "/var/lib/ntopng"]
    restart: unless-stopped
  ...
  keycloak-db:
    ...
    container_name: discovery-ova-keycloak-db
    environment:
      TZ: \${UTC_TZ}
  ...
  keycloak:
    ...
    container_name: discovery-ova-keycloak
    depends_on:
    ...
    environment:
      ...
      KC_BOOTSTRAP_ADMIN_USERNAME: \${GRAFANA_ADMIN_USER}
      KC_BOOTSTRAP_ADMIN_PASSWORD: \${OPERATOR_PASSWORD}
    ...
    volumes:
      - $CONFIG_DIR/keycloak/realm.json:/opt/keycloak/data/import/realm.json:ro
    ports:
      - "127.0.0.1:8088:8080"
    restart: unless-stopped
  ...
  oauth2-proxy:
    ...
    container_name: discovery-ova-oauth2-proxy
    network_mode: host
    environment:
      TZ: \${UTC_TZ}
    command: ["--config=/etc/oauth2-proxy/oauth2-proxy.cfg"]
    volumes:
      - $CONFIG_DIR/oauth2-proxy/oauth2-proxy.cfg:/etc/oauth2-proxy/oauth2-proxy.cfg:ro
    depends_on:
      - keycloak
    restart: unless-stopped
...
EOF
  ...
  local url
  for url in http://127.0.0.1:8001 http://127.0.0.1:8002 http://127.0.0.1:8003 http://127.0.0.1:3000 http://127.0.0.1:3001 http://127.0.0.1:8085/guacamole/ http://127.0.0.1:9115/metrics http://127.0.0.1:8088/auth/realms/discovery-ova/.well-known/openid-configuration; do
    for _ in {1..60}; do
...
<a class="card" href="/auth/admin/discovery-ova/console/" target="_blank" rel="noopener noreferrer"><b>Identity / SSO</b><span>Keycloak users, sessions and single sign-on configuration.</span><i class="tag">identity</i></a>
<a class="card" href="/oauth2/sign_out"><b>Sign Out</b><span>End the Discovery-OVA SSO session.</span><i class="tag">security</i></a>
</div></section>
...
EOF
  ...
  write_file /usr/local/sbin/discovery-ova-sso-refresh 0750 root:root <<'EOF'
#!/usr/bin/env bash
set -Eeuo pipefail
source /etc/discovery-ova/discovery-ova.conf
source /etc/discovery-ova/secrets.env
ip="$(ip -4 -o addr show dev "$MGMT_IF" scope global 2>/dev/null | awk 'NR==1{split($4,a,"/");print a[1]}')"
[[ -n "$ip" ]] || exit 1
cfg=/opt/discovery-ova/config/oauth2-proxy/oauth2-proxy.cfg
[[ -r "$cfg" ]] || exit 1
sed -i \
  -e "s#^oidc_issuer_url = .*#oidc_issuer_url = \"https://$ip/auth/realms/discovery-ova\"#" \
  -e "s#^redirect_url = .*#redirect_url = \"https://$ip/oauth2/callback\"#" "$cfg"
chmod 0600 "$cfg"

# Keep the Keycloak client callback synchronized with DHCP/clone address changes.
for _ in {1..60}; do
  curl -fsS http://127.0.0.1:8088/auth/realms/master >/dev/null 2>&1 && break
  sleep 2
done
if docker exec discovery-ova-keycloak /opt/keycloak/bin/kcadm.sh config credentials \
    --server http://127.0.0.1:8080/auth --realm master --user "$OPERATOR_USER" --password "$OPERATOR_PASSWORD" >/dev/null 2>&1; then
  cid="$(docker exec discovery-ova-keycloak /opt/keycloak/bin/kcadm.sh get clients -r discovery-ova -q clientId=discovery-ova-proxy 2>/dev/null | jq -r '.[0].id // empty')"
  if [[ -n "$cid" ]]; then
    docker exec discovery-ova-keycloak /opt/keycloak/bin/kcadm.sh update "clients/$cid" -r discovery-ova \
      -s "redirectUris=[\"https://$ip/oauth2/callback\"]" \
      -s "webOrigins=[\"https://$ip\"]" >/dev/null
  fi
fi

docker restart discovery-ova-oauth2-proxy >/dev/null 2>&1 || true
logger -t discovery-ova-sso "SSO callback synchronized to management IP $ip"
EOF

  write_file /etc/systemd/system/discovery-ova-sso-refresh.service 0644 root:root <<'EOF'
[Unit]
Description=Synchronize Discovery-OVA SSO callback URLs with current management IP
After=docker.service nginx.service discovery-ova-tls-refresh.service network-online.target
Requires=docker.service

[Service]
Type=oneshot
ExecStart=/usr/local/sbin/discovery-ova-sso-refresh
TimeoutStartSec=180
...
EOF
  systemctl daemon-reload
  systemctl enable discovery-ova-tls-refresh.service discovery-ova-sso-refresh.service
  /usr/local/sbin/discovery-ova-sso-refresh || warn "Initial SSO callback refresh did not complete; validation will retry."
  local sso_ready=0 portal_code
  for _ in {1..60}; do
    if curl -fsS http://127.0.0.1:4180/ping >/dev/null 2>&1; then sso_ready=1; break; fi
    sleep 2
  done
  (( sso_ready == 1 )) || die "OAuth2 Proxy did not become ready after Keycloak/Nginx startup."
  portal_code="$(curl -ksS --max-time 8 -o /dev/null -w '%{http_code}' https://127.0.0.1/)"
  if [[ "$portal_code" != 302 && "$portal_code" != 401 ]]; then
    log "SSO portal validation failed with HTTP $portal_code; collecting Nginx diagnostics."
    nginx -T >>"$LOG_FILE" 2>&1 || true
    tail -n 80 /var/log/nginx/error.log | tee -a "$LOG_FILE" || true
    die "HTTPS SSO portal validation failed. See $LOG_FILE and /var/log/nginx/error.log."
  fi
  curl -fsS http://127.0.0.1:8088/auth/realms/discovery-ova/.well-known/openid-configuration >/dev/null     || die "Keycloak Discovery-OVA realm discovery endpoint is unavailable."
  log "HTTPS portal and local Keycloak/OAuth2 SSO validated by IP; no DNS/mDNS required."
}
write_control_app() {
...
"${compose[@]}" stop dispatcher librenms db guacamole guacamole-db keycloak keycloak-db >/dev/null || true
include=(
  ...
  /opt/discovery-ova/data/keycloak-db
  ...
  /etc/update-motd.d/99-discovery-ova
  /etc/issue.d/discovery-ova.issue
  /etc/netplan
  /etc/systemd/system
  ...
  /usr/local/sbin/discovery-ova-sso-refresh
  /usr/local/sbin/discovery-ova-iperf3-server
  /usr/local/sbin/discovery-ova-backup
  /usr/local/sbin/discovery-ova-firstboot
)
# Keep a second copy of Discovery-OVA-owned NetworkManager profiles inside the appliance config tree.
install -d -m 0700 /opt/discovery-ova/config/network
if [[ -r /etc/NetworkManager/system-connections/discovery-ova-capture.nmconnection ]]; then
  install -m 0600 /etc/NetworkManager/system-connections/discovery-ova-capture.nmconnection /opt/discovery-ova/config/network/discovery-ova-capture.nmconnection
fi
if [[ -r /etc/NetworkManager/system-connections/discovery-ova-wired.nmconnection ]]; then
  install -m 0600 /etc/NetworkManager/system-connections/discovery-ova-wired.nmconnection /opt/discovery-ova/config/network/discovery-ova-wired.nmconnection
fi
...
initialize_app_accounts() {
  stage 9 "Application account and SSO bootstrap"
  ...
  # Trust the authenticated identity injected by the local Nginx/OAuth2 Proxy path.
  # The LibreNMS backend is bound to host loopback only; users cannot reach it directly.
  docker exec -u librenms discovery-ova-librenms /opt/librenms/lnms config:set auth_mechanism sso >/dev/null || warn "LibreNMS SSO auth mechanism could not be enabled."
  docker exec -u librenms discovery-ova-librenms /opt/librenms/lnms config:set sso.mode env >/dev/null || true
  docker exec -u librenms discovery-ova-librenms /opt/librenms/lnms config:set sso.user_attr HTTP_X_REMOTE_USER >/dev/null || true
  docker exec -u librenms discovery-ova-librenms /opt/librenms/lnms config:set sso.group_strategy static >/dev/null || true
  docker exec -u librenms discovery-ova-librenms /opt/librenms/lnms config:set sso.static_level 10 >/dev/null || true
  docker exec -u librenms discovery-ova-librenms /opt/librenms/lnms config:set sso.create_users true >/dev/null || true
  docker exec -u librenms discovery-ova-librenms /opt/librenms/lnms config:set sso.update_users true >/dev/null || true
  docker exec -u librenms discovery-ova-librenms /opt/librenms/lnms config:set sso.trusted_proxies '["127.0.0.1/8","172.16.0.0/12"]' >/dev/null || true
  docker exec -u librenms discovery-ova-librenms /opt/librenms/lnms config:set sso.auth_logout_handler /oauth2/sign_out >/dev/null || true
  ...
  log "ntopng local login is disabled; access is enforced by the Discovery-OVA Keycloak/OAuth2 Proxy SSO gate."
}
...
rekey() {
  ...
  if docker ps --format '{{.Names}}' | grep -Fxq discovery-ova-keycloak; then
    if docker exec discovery-ova-keycloak /opt/keycloak/bin/kcadm.sh config credentials --server http://127.0.0.1:8080/auth --realm master --user "$OPERATOR_USER" --password "$OPERATOR_PASSWORD" >/dev/null 2>&1; then
      docker exec discovery-ova-keycloak /opt/keycloak/bin/kcadm.sh set-password -r discovery-ova --username "$OPERATOR_USER" --new-password "$new_password" >/dev/null 2>&1         || warn "Keycloak Discovery-OVA realm password could not be updated automatically."
      docker exec discovery-ova-keycloak /opt/keycloak/bin/kcadm.sh set-password -r master --username "$OPERATOR_USER" --new-password "$new_password" >/dev/null 2>&1         || warn "Keycloak master admin password could not be updated automatically."
    else
      warn "Could not authenticate to Keycloak for SSO password rotation."
    fi
  fi
  ...
  systemctl reload nginx
  log "Operator credential rotated for Keycloak SSO and local fallback accounts where reachable."
}
...
validate() {
  ...
  if [[ "$(curl -ksS -o /dev/null -w "%{http_code}" https://127.0.0.1/)" =~ ^(302|401)$ ]]; then pass "Portal SSO authentication challenge"; else fail "Portal SSO authentication challenge"; fi
  check "Keycloak realm discovery" curl -fsS --max-time 8 http://127.0.0.1:8088/auth/realms/discovery-ova/.well-known/openid-configuration
  check "OAuth2 Proxy ready" curl -fsS --max-time 8 http://127.0.0.1:4180/ping
  ...
  check "SSO refresh unit installed" systemctl cat discovery-ova-sso-refresh.service

  local c
  for c in discovery-ova-db discovery-ova-redis discovery-ova-librenms discovery-ova-librenms-dispatcher discovery-ova-smokeping discovery-ova-librespeed discovery-ova-ntopng discovery-ova-guacamole-db discovery-ova-guacd discovery-ova-guacamole discovery-ova-keycloak-db discovery-ova-keycloak discovery-ova-oauth2-proxy discovery-ova-blackbox-exporter discovery-ova-prometheus discovery-ova-node-exporter discovery-ova-cadvisor discovery-ova-loki discovery-ova-alloy discovery-ova-grafana; do
  ...
  check "Dynamic login banner installed" test -x /etc/update-motd.d/99-discovery-ova
  check "Login banner renders management URL" bash -c '/etc/update-motd.d/99-discovery-ova | grep -Fq "https://"'
  ...
  loki_ok=0
  for _ in {1..30}; do curl -fsS --max-time 3 http://127.0.0.1:3100/ready >/dev/null 2>&1 && { loki_ok=1; break; }; sleep 2; done
  (( loki_ok == 1 )) && pass "Loki ready" || fail "Loki ready"
  ...
  if [[ -L "$latest" || -f "$latest" ]] && [[ -f "$BACKUP_DIR/local/latest.tar.gz.sha256" ]]; then
    ...
    if grep -Fq 'etc/NetworkManager/system-connections/discovery-ova-capture.nmconnection' <<<"$archive_list" || grep -Fq 'opt/discovery-ova/config/network/discovery-ova-capture.nmconnection' <<<"$archive_list"; then pass "Backup contains capture NetworkManager profile"; else fail "Backup contains capture NetworkManager profile"; fi
    ...
    grep -Fq 'opt/discovery-ova/data/keycloak-db/' <<<"$archive_list" && pass "Backup contains Keycloak SSO database state" || fail "Backup contains Keycloak SSO database state"
    ...
    if grep -Fxq 'etc/discovery-ova/backup_ed25519' <<<"$archive_list"; then fail "Backup excludes private remote-backup key"; else pass "Backup excludes private remote-backup key"; fi
  else
...
Discovery-OVA is ready.
  ...
  SSO:      https://$ip/auth/realms/discovery-ova/account/
  Identity: https://$ip/auth/admin/discovery-ova/console/

Authentication is centralized through local Keycloak SSO using the '$OPERATOR_USER' account.
