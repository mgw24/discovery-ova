...
KEYCLOAK_DB_IMAGE="postgres:17.6-alpine"
...
collect_settings() {
  ...
  SSO_CLIENT_SECRET="${SSO_CLIENT_SECRET:-$(random_secret)}"
  # oauth2-proxy requires exactly 16, 24 or 32 bytes for its AES cookie secret.
  OAUTH2_COOKIE_SECRET="${OAUTH2_COOKIE_SECRET:-$(openssl rand -hex 16)}"
  [[ ${#OAUTH2_COOKIE_SECRET} -eq 32 ]] || OAUTH2_COOKIE_SECRET="$(openssl rand -hex 16)"
...
ensure_dirs_user() {
  ...
  install -d -m 0755 -o root -g root "$CONFIG_DIR/oauth2-proxy"
  install -d -m 0750 -o root -g root "$CONFIG_DIR/keycloak" "$DATA_DIR/keycloak-db"
  chown 70:70 "$DATA_DIR/keycloak-db"
...
EOF
  ...
  local sso_ip
  sso_ip="$(get_mgmt_ip "$MGMT_IF")"
  [[ -n "$sso_ip" ]] || die "Cannot configure SSO without a management IPv4 address."

  # The OAuth2 Proxy config intentionally contains no credentials and is world-readable
  # inside its isolated bind mount. Secrets are passed through the container environment.
  # This avoids the unprivileged oauth2-proxy UID failing to read a root-owned config.
  cat > "$CONFIG_DIR/oauth2-proxy/oauth2-proxy.cfg" <<EOF_SSO
provider = "oidc"
# Browser-visible issuer remains management-IP based; normal operation does not require DNS.
oidc_issuer_url = "https://$sso_ip/auth/realms/discovery-ova"
redirect_url = "https://$sso_ip/oauth2/callback"
# Avoid Docker-to-host hairpin OIDC discovery. Browser authorization uses the public URL,
# while token, JWKS and userinfo traffic stays on the private Docker network.
skip_oidc_discovery = true
login_url = "https://$sso_ip/auth/realms/discovery-ova/protocol/openid-connect/auth"
redeem_url = "http://keycloak:8080/auth/realms/discovery-ova/protocol/openid-connect/token"
oidc_jwks_url = "http://keycloak:8080/auth/realms/discovery-ova/protocol/openid-connect/certs"
profile_url = "http://keycloak:8080/auth/realms/discovery-ova/protocol/openid-connect/userinfo"
client_id = "discovery-ova"
email_domains = [ "*" ]
...
EOF_SSO
  ...
  [[ $(stat -c '%a' "$CONFIG_DIR/oauth2-proxy/oauth2-proxy.cfg") == 644 ]] || die "OAuth2 Proxy config permissions are invalid."
  [[ ${#OAUTH2_COOKIE_SECRET} -eq 32 ]] || die "OAuth2 cookie secret must be exactly 32 bytes."

  jq -n --arg user "$OPERATOR_USER" --arg pass "$OPERATOR_PASSWORD" --arg client_secret "$SSO_CLIENT_SECRET" --arg ip "$sso_ip" '{
    realm:"discovery-ova", enabled:true, displayName:"Discovery-OVA",
    registrationAllowed:false, resetPasswordAllowed:false, rememberMe:true,
    clients:[{clientId:"discovery-ova",name:"Discovery-OVA",enabled:true,protocol:"openid-connect",publicClient:false,secret:$client_secret,standardFlowEnabled:true,directAccessGrantsEnabled:false,redirectUris:[("https://"+$ip+"/oauth2/callback")],webOrigins:[("https://"+$ip)]}],
    users:[{username:$user,enabled:true,emailVerified:true,firstName:"Discovery",lastName:"Operator",credentials:[{type:"password",value:$pass,temporary:false}]}]
  }' > "$CONFIG_DIR/keycloak/discovery-ova-realm.json"
  chmod 0400 "$CONFIG_DIR/keycloak/discovery-ova-realm.json"
  chown 1000:1000 "$CONFIG_DIR/keycloak/discovery-ova-realm.json"
...
services:
  ...
  keycloak:
    ...
    environment:
      ...
      KC_HOSTNAME: https://${sso_ip}/auth
      KC_BOOTSTRAP_ADMIN_USERNAME: \${GRAFANA_ADMIN_USER}
      KC_BOOTSTRAP_ADMIN_PASSWORD: \${OPERATOR_PASSWORD}
    command: ["start-dev", "--import-realm"]
    volumes:
      - $CONFIG_DIR/keycloak/discovery-ova-realm.json:/opt/keycloak/data/import/discovery-ova-realm.json:ro
    ports:
      - "127.0.0.1:8088:8080"
    restart: unless-stopped
...
EOF
  ...
  log "Validating LibreNMS internal Docker DNS before application readiness checks."
  local dns_ok=0
  for _ in {1..20}; do
    if docker run --rm --network discovery-ova-internal --entrypoint getent "$LIBRENMS_IMAGE" hosts redis >/dev/null 2>&1 \
       && docker run --rm --network discovery-ova-internal --entrypoint getent "$LIBRENMS_IMAGE" hosts db >/dev/null 2>&1; then
      dns_ok=1
      break
    fi
    sleep 2
  ...
  if (( dns_ok == 0 )); then
    warn "LibreNMS Docker DNS validation failed. Internal network inspection follows:"
    docker network inspect discovery-ova-internal 2>&1 | tee -a "$LOG_FILE" || true
    docker logs --tail 100 discovery-ova-redis 2>&1 | tee -a "$LOG_FILE" || true
    die "LibreNMS cannot resolve its Redis/database service names on discovery-ova-internal."
  fi

  log "Waiting for LibreNMS HTTP backend."
  local librenms_ready=0
  for _ in {1..60}; do
    if curl -fsS --max-time 3 http://127.0.0.1:8001/ >/dev/null 2>&1; then
      librenms_ready=1
      break
    fi
    sleep 3
  done
  if (( librenms_ready == 0 )); then
    warn "LibreNMS did not become ready. Container and network diagnostics follow:"
    docker ps -a --filter name=discovery-ova-librenms 2>&1 | tee -a "$LOG_FILE" || true
    docker logs --tail 150 discovery-ova-librenms 2>&1 | tee -a "$LOG_FILE" || true
    docker logs --tail 100 discovery-ova-librenms-dispatcher 2>&1 | tee -a "$LOG_FILE" || true
    docker network inspect discovery-ova-internal 2>&1 | tee -a "$LOG_FILE" || true
    die "LibreNMS backend did not become ready on 127.0.0.1:8001."
  fi

  log "Waiting for remaining core HTTP endpoints."
  local url
  for url in http://127.0.0.1:8002 http://127.0.0.1:8003 http://127.0.0.1:3000 http://127.0.0.1:3001 http://127.0.0.1:8085/guacamole/ http://127.0.0.1:9115/metrics; do
    for _ in {1..60}; do
...
write_nginx_proxy_server() {
  local port="$1" upstream="$2" label="$3" extra_server=""
  if [[ "$label" == "LibreSpeed" ]]; then
    extra_server="    client_max_body_size 100m;"
  fi
  cat <<EOF
server {
    listen $port ssl;
...
$extra_server
    ...
    location /oauth2/ { proxy_pass http://127.0.0.1:4180; proxy_set_header Host \$host; proxy_set_header X-Real-IP \$remote_addr; proxy_set_header X-Forwarded-Proto https; }
    location / {
...
<a class="card" href="/auth/realms/discovery-ova/account/" target="_blank" rel="noopener noreferrer"><b>Identity / SSO</b><span>Discovery-OVA Keycloak account and SSO session management.</span><i class="tag">identity</i></a>
</div></section>
...
server {
    ...
    location / { auth_request /oauth2/auth; error_page 401 = /oauth2/start?rd=\$scheme://\$http_host\$request_uri; try_files \$uri \$uri/ /index.html; }
    location /scanner/ { auth_request /oauth2/auth; error_page 401 = /oauth2/start?rd=\$scheme://\$http_host\$request_uri; proxy_pass http://127.0.0.1:8787; include proxy_params; proxy_read_timeout 920s; }
    location /network/ { auth_request /oauth2/auth; error_page 401 = /oauth2/start?rd=\$scheme://\$http_host\$request_uri; proxy_pass http://127.0.0.1:8787; include proxy_params; }
    location /capture/ { auth_request /oauth2/auth; error_page 401 = /oauth2/start?rd=\$scheme://\$http_host\$request_uri; proxy_pass http://127.0.0.1:8787; include proxy_params; proxy_read_timeout 650s; }
    location /captures/ { auth_request /oauth2/auth; error_page 401 = /oauth2/start?rd=\$scheme://\$http_host\$request_uri; proxy_pass http://127.0.0.1:8787; include proxy_params; }
    location /diagnostics/ { auth_request /oauth2/auth; error_page 401 = /oauth2/start?rd=\$scheme://\$http_host\$request_uri; proxy_pass http://127.0.0.1:8787; include proxy_params; proxy_read_timeout 90s; }
    location /remote/ { auth_request /oauth2/auth; error_page 401 = /oauth2/start?rd=\$scheme://\$http_host\$request_uri; proxy_pass http://127.0.0.1:8787; include proxy_params; }
    location /api/ { auth_request /oauth2/auth; error_page 401 = /oauth2/start?rd=\$scheme://\$http_host\$request_uri; proxy_pass http://127.0.0.1:8787; include proxy_params; }
    location /guacamole/ {
        auth_request /oauth2/auth;
        error_page 401 = /oauth2/start?rd=\$scheme://\$http_host\$request_uri;
        auth_request_set \$auth_user \$upstream_http_x_auth_request_user;
    ...
    location /status/ { auth_request /oauth2/auth; error_page 401 = /oauth2/start?rd=\$scheme://\$http_host\$request_uri; proxy_pass http://127.0.0.1:8787; include proxy_params; }
...
EOF
  ...
  log "Waiting for Keycloak and OAuth2 Proxy SSO readiness."
  local sso_ready=0
  local keycloak_host_ready=0
  for _ in {1..60}; do
    if curl -fsS --max-time 3 http://127.0.0.1:8088/auth/realms/discovery-ova/.well-known/openid-configuration >/dev/null 2>&1; then
      keycloak_host_ready=1
      break
    fi
    sleep 3
  done
  (( keycloak_host_ready == 1 )) || die "Keycloak did not become reachable on the host-local port."

  # Validate the same Docker-internal JWKS endpoint used by OAuth2 Proxy.
  local compose_network="discovery-ova_default"
  local internal_oidc_ok=0
  for _ in {1..20}; do
    if docker run --rm --network "$compose_network" "$CURL_IMAGE" \
         -fsS --max-time 4 http://keycloak:8080/auth/realms/discovery-ova/protocol/openid-connect/certs >/dev/null 2>&1; then
      internal_oidc_ok=1
      break
    fi
    sleep 2
  done
  if (( internal_oidc_ok == 0 )); then
    warn "Keycloak is reachable from the host but its Docker-internal OIDC endpoint is not reachable."
    docker network inspect "$compose_network" 2>&1 | tee -a "$LOG_FILE" >/dev/null || true
    die "Docker-internal Keycloak OIDC connectivity failed."
  fi

  for _ in {1..60}; do
    if curl -fsS --max-time 3 http://127.0.0.1:4180/ping >/dev/null 2>&1; then
      sso_ready=1
      break
    fi
    sleep 3
  ...
  if (( sso_ready == 0 )); then
    warn "SSO readiness failed. OAuth2 Proxy logs follow:"
    docker logs --tail 120 discovery-ova-oauth2-proxy 2>&1 | tee -a "$LOG_FILE" || true
    warn "Keycloak logs follow:"
    docker logs --tail 120 discovery-ova-keycloak 2>&1 | tee -a "$LOG_FILE" || true
    die "OAuth2 Proxy did not become ready after Keycloak startup."
  fi
  log "Keycloak and OAuth2 Proxy are ready."
...
Restart=on-failure
RestartSec=3
RuntimeDirectory=discovery-ova
...
EOF
  ...
  local web_ready=0
  for _ in {1..20}; do
    if curl -fsS --max-time 3 http://127.0.0.1:8787/health >/tmp/discovery-ova-web-health.json 2>/dev/null; then
      web_ready=1
      break
    fi
    sleep 2
  ...
  if (( web_ready == 0 )); then
    warn "Discovery-OVA web backend failed to start. systemd diagnostics follow:"
    systemctl status discovery-ova-web.service --no-pager -l 2>&1 | tee -a "$LOG_FILE" || true
    journalctl -u discovery-ova-web.service -n 100 --no-pager 2>&1 | tee -a "$LOG_FILE" || true
    die "Discovery-OVA web backend did not become ready on 127.0.0.1:8787."
  fi
  jq . /tmp/discovery-ova-web-health.json | tee -a "$LOG_FILE"
  rm -f /tmp/discovery-ova-web-health.json
}
configure_metrics() {
...
validate() {
  ...
  check "Portal SSO redirect" bash -c 'c="$(curl -ksS -o /dev/null -w "%{http_code}" https://127.0.0.1/)"; [[ "$c" == 302 || "$c" == 303 ]]'
  ...
  check "Scanner page backend" curl -fsS --max-time 8 http://127.0.0.1:8787/scanner/
  check "Remote probes page backend" curl -fsS --max-time 8 http://127.0.0.1:8787/remote/
  check "Packet capture page backend" curl -fsS --max-time 8 http://127.0.0.1:8787/capture/
  check "Network diagnostics page backend" curl -fsS --max-time 8 http://127.0.0.1:8787/diagnostics/
  check "Network control page backend" curl -fsS --max-time 8 http://127.0.0.1:8787/network/
  check "Appliance status page backend" curl -fsS --max-time 8 http://127.0.0.1:8787/status/
  ...
  check "Keycloak realm ready" curl -fsS --max-time 8 http://127.0.0.1:8088/auth/realms/discovery-ova/.well-known/openid-configuration
  check "Keycloak Docker-internal JWKS endpoint" docker run --rm --network discovery-ova_default "$CURL_IMAGE" -fsS --max-time 8 http://keycloak:8080/auth/realms/discovery-ova/protocol/openid-connect/certs
